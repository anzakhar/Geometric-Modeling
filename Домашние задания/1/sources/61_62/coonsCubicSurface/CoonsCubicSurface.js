...
const Data = {
    ...
    generateControlPoints: function () {
        ...
        this.create_coord_tip("10", this.heighTip, N_ctr, M_ctr);
        this.create_coord_tip("01", this.heighTip, N_ctr, M_ctr);
        this.create_coord_tip("11", this.heighTip, N_ctr, M_ctr);

        this.create_indexes_tip("10", N_ctr, M_ctr);
        this.create_indexes_tip("01", N_ctr, M_ctr);
        this.create_indexes_tip("11", N_ctr, M_ctr);
	...
	setVector: function (x1, y1, z1, x2, y2, z2, orient, create, i, j) {
        ...
        if ((orient != "normals") && (orient != "tangents1") && (orient != "tangents2")) {
        ...
        if (this.leftButtonDown) {
            ...
            for (let i = 0; i < this.controlsParameters.N_ctr; i++)
                for (let j = 0; j < this.controlsParameters.M_ctr; j++) {
                    ...
                    if (((j == 0) || (j == this.controlsParameters.M_ctr - 1)) && (this.m01PointsCtr[i][j].ptInRect(x, y)))
    ...
    setVertexBuffersAndDraw: function () {
        ...
        for (i = 0; i < N_ctr; i++)
            for (j = 0; j < M_ctr; j++) {
                ...
                if (this.controlsParameters.showCtrPoints)
                	this.gl.drawArrays(this.gl.POINTS, i*M_ctr + j, 1);
            }
        ...
        if (this.controlsParameters.controlNet) {
            ...
            this.gl.drawElements(this.gl.LINE_STRIP, M_ctr, this.gl.UNSIGNED_SHORT, 0);
            
            this.gl.uniform4f(this.u_color, 0.0, 1.0, 1.0, 1.0);
            this.gl.uniform4f(this.u_colorSelect, 0.0, 1.0, 1.0, 1.0);
            this.gl.drawElements(this.gl.LINE_STRIP, M_ctr, this.gl.UNSIGNED_SHORT, ((N_ctr-1) * M_ctr) * this.ISIZE);

            this.gl.uniform4f(this.u_color, 0.0, 0.0, 1.0, 1.0);
            this.gl.uniform4f(this.u_colorSelect, 0.0, 0.0, 1.0, 1.0);
            this.gl.drawElements(this.gl.LINE_STRIP, N_ctr, this.gl.UNSIGNED_SHORT, ((N_ctr * M_ctr) * this.ISIZE));
            
            this.gl.uniform4f(this.u_color, 1.0, 0.0, 1.0, 1.0);
            this.gl.uniform4f(this.u_colorSelect, 1.0, 0.0, 1.0, 1.0);
            this.gl.drawElements(this.gl.LINE_STRIP, N_ctr, this.gl.UNSIGNED_SHORT, ((N_ctr * M_ctr + (M_ctr-1) * N_ctr) * this.ISIZE));
        }
        ...
        if (this.controlsParameters.showCtrPoints) {
	        ...
	        for (i = 0; i < 3; i++) {
	            ...
	            this.gl.drawArrays(this.gl.LINES, 0, 2 * N_ctr * M_ctr);
	        }
	        ...
	        for (i = 0; i < 3; i++) {
	            ...
	            this.gl.drawElements(this.gl.TRIANGLES, N_ctr * M_ctr * countIndicesOneTip, this.gl.UNSIGNED_SHORT, 0);
	        }
      	}
        if (this.controlsParameters.coonsCubicSurface) {
