...
const Data = {
    ...
    init: function (gl, viewport) {
        ...
        this.verticesCtr = new Float32Array((this.N_ctr + 1) * this.countAttribData);
        this.createIndicesCtr(this.N_ctr);
        this.ISIZE = this.indicesCtr.BYTES_PER_ELEMENT;
        this.setDependentGeomParameters(this.controlsParameters.x0, this.controlsParameters.y0, this.controlsParameters.radius);
    },
    setDependentGeomParameters: function (x0, y0, radius) {

		const Xmin = Math.min(x0 - radius, x0 - radius);
        const Xmax = Math.max(x0 + radius, x0 + radius);
        const Ymin = Math.min(y0 - radius, y0 - radius);
        const Ymax = Math.max(y0 + radius, y0 + radius);
    ...
    setVertexBuffersAndDraw: function () {
        ...
        if (this.controlsParameters.controlNet) {
            ...
            for (i = 0; i < this.N_ctr; i++)
                this.gl.drawElements(this.gl.LINES, 2, this.gl.UNSIGNED_SHORT, (this.N_ctr + i * 2) * this.ISIZE);
        }
        if (this.controlsParameters.sectorialSurface)
        {
