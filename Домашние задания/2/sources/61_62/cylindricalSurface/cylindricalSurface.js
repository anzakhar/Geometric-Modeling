...
const Data = {
    ...
    setDependentGeomParameters: function () {
		const Xmin = this.controlsParameters.Xmin,
			  ...
			  dx = this.controlsParameters.dx, 
			  dy = this.controlsParameters.dy, 
			  dz = this.controlsParameters.dz; 
        this.Xmid = Xmin + (Xmax - Xmin) / 2.0;
		
        let Xmin1 = Math.min(Xmin - this.Xmid, Xmin - this.Xmid + dx);
        let Xmax1 = Math.max(Xmax - this.Xmid, Xmax - this.Xmid + dx);

        Camera.d0 = Math.sqrt(Math.pow(Math.max(Math.abs(Xmin1), Math.abs(Xmax1)), 2) +
			Math.pow(Math.max(Math.abs(Y), Math.abs(Y + dy)), 2) +
           Math.pow(Math.max(Math.abs(Z), Math.abs(Z + dz)), 2));
    ...
    add_vertices: function (n) {
        ...
        this.verticesCtr = new Float32Array(n * 4);
        for (let i = 0; i < n; i++) {
    ...
    setVertexBuffersAndDraw: function () {
        ...
        if (this.controlsParameters.showCtrPoints)
        		this.gl.drawArrays(this.gl.POINTS, 0, this.controlsParameters.N_ctr);
